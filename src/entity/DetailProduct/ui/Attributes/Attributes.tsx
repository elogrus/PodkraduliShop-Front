...
export const Attributes = (props: AttributesProps) => {
    ...
    return (
        <div className={cmcl(cls.Attributes, {}, [className])} {...otherProps}>
            <Text preset={TextPreset.SUBTITLE} mods={[TextMods.BOLD]}>О товаре</Text>
            <div className={cls.List}>
                {product.attributes.map((attr) => <div key={attr.title} className={cls.ListItem}>
                    <Text preset={TextPreset.REGULAR}>{attr.title}</Text>
                    <Text className={cls.Value} preset={TextPreset.REGULAR}>{typeof attr.value === "boolean"
                        ? (attr.value ? 'Да' : 'Нет')
                        : attr.value}
                    </Text>
                </div>)}
